import json
from src.ingestion.movie_provider_ingestor import MovieProvidersIngestor
...
def lambda_handler(event, context):
    ...
    movie_providers = MovieProvidersIngestor(tmdb_fetcher=tmdb_fetcher, s3_conn=s3_conn)
    movie_providers.ingest(movie_ids_key=key)
    return {
        "statusCode": 200,
        "body": json.dumps({"message": "Movie providers sucessfully written to S3"}),
    }
