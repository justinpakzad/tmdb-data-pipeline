import json
from src.ingestion.movie_reviews_ingestor import MovieReviewsIngestor
...
def lambda_handler(event, context):
    ...
    movie_reviews = MovieReviewsIngestor(tmdb_fetcher=tmdb_fetcher, s3_conn=s3_conn)
    movie_reviews.ingest(movie_ids_key=key)
    return {
        "statusCode": 200,
        "body": json.dumps({"message": "Movie reviews sucessfully written to S3"}),
    }
