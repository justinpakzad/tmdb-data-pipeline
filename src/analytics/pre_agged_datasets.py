...
def movies_by_year_and_genre(
    df_movies: pl.DataFrame, df_reviews: pl.DataFrame
) -> pl.DataFrame:
    return (
...
def country_level_movie_stats(df_movies: pl.DataFrame) -> pl.DataFrame:
    return (
        df_movies.explode("origin_country")
...
def cast_metrics(
    df_cast: pl.DataFrame, df_reviews: pl.DataFrame, df_movies: pl.DataFrame
):
    df_most_pop_genre = (
        df_cast.join(df_movies.explode("genres"), on="movie_id")
    ...
    return (
        df_cast.join(df_movies, on="movie_id")
        .join(df_reviews, on="movie_id", how="left")
        .group_by("name")
        .agg(
            pl.col("movie_id").n_unique().alias("movie_count"),
...
def directors_metrics(
    df_crew: pl.DataFrame, df_movies: pl.DataFrame, df_reviews: pl.DataFrame
):
    return (
...
def get_agged_dfs():
    ...
    df_movies = pl.read_parquet("s3://tmdb-data-jp/processed/movie_details/")
    ...
    return {
        "agg_movies_by_year_and_genre": movies_by_year_and_genre(
            df_movies=df_movies, df_reviews=df_reviews
        ),
        "agg_country_level_movie_stats": country_level_movie_stats(df_movies=df_movies),
        "agg_cast_metrics": cast_metrics(
            df_cast=df_cast, df_reviews=df_reviews, df_movies=df_movies
        ),
        "agg_director_metrics": directors_metrics(
            df_crew=df_crew, df_movies=df_movies, df_reviews=df_reviews
        ),
